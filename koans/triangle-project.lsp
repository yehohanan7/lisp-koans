...
(defun isoscelesp (a b c)
  (let ((sides (sort (list a b c) #'<))) 
    (= (second sides) (third sides))))


(defun scalene (a b c)
  (let ((sides (list a b c))) 
    (equal sides (remove-duplicates sides))))

(defun equilateral (a b c)
  (= a b c))

(defun invalidp (a b c)
  (loop for x in (list a b c)
        when (<= x 0) return x))

(defun triangle (a b c)
  (cond
    ((invalidp a b c) (make-condition 'triangle-error))
    ((equilateral a b c) :equilateral)
    ((isoscelesp a b c) :isosceles)
    ((scalene a b c) :scalene)
    (t (make-condition 'triangle-error))))
